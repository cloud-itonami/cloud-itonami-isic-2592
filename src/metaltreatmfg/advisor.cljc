(ns metaltreatmfg.advisor
  "MetalTreatmentAdvisor -- the *contained intelligence node* for the
  metal-treatment/coating/machining job-shop plant-operations
  coordination actor (ISIC 2592, treatment and coating of metals;
  machining).

  It normalizes production-batch patches (product-category/weight/
  defect-rate), drafts an electroplating-bath/anodizing-tank/heat-
  treatment-furnace/CNC-machine maintenance scheduling proposal
  against a piece of equipment, drafts a safety-concern flag, and
  drafts an outbound electroplated/anodized/coated/galvanized/heat-
  treated/machined product shipment coordination proposal against a
  ...
  a committed record and NEVER a real plating/machining-line
  actuation or freight dispatch. Every output is censored downstream
  by `metaltreatmfg.governor` before anything touches the SSoT -- see
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `metaltreatmfg.governor`)
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct plating/
                                 ; machining-line-control effect
  ...
  `metaltreatmfg.governor` HARD-holds any request that doesn't, so a
...
(defn- log-production-batch
  "Production-batch (electroplated/anodized/coated/galvanized/heat-
  treated/machined lot) intake upsert -- the advisor only normalizes/
  validates the patch; it does not invent the batch's product-
  category, weight, defect-rate or verification status. High
...
(defn- schedule-maintenance
  "Draft an electroplating-bath/anodizing-tank/heat-treatment-furnace/
  CNC-machine maintenance-window scheduling proposal against a piece
  of equipment. The advisor reports what it can see (equipment
  verified?/registered?) in its rationale, but `metaltreatmfg.governor`
    ...
    {:summary    (str subject " 向け保守作業予定提案 (" (:maintenance-type value) ")"
     ...
     :rationale  (if eq
                   (str "equipment-verified?=" (registry/equipment-verified? eq)
                        ...
                        " actuate-plating-machining-line?=" (boolean (:actuate-plating-machining-line? value)))
                   (str equipment-id " が見つかりません"))
     :cites      (if eq [equipment-id] [])
     :effect     :maintenance/schedule
     :value      value
     :stake      nil
     :confidence (if (and ready? (not (:actuate-plating-machining-line? value))) 0.9 0.3)}))

(defn- flag-safety-concern
  "Draft an electroplating-bath chemical-toxicity (cyanide/hexavalent-
  chromium/acid bath)/CNC-machining-swarf-cutting-tool-laceration/
  heat-treatment-furnace-burn-radiant-heat/equipment-safety concern.
  ...
  technicality). See `metaltreatmfg.phase`: no phase ever adds this op
  to a phase's `:auto` set; `metaltreatmfg.governor` also always
...
(defn- coordinate-shipment
  "Draft an outbound electroplated/anodized/coated/galvanized/heat-
  treated/machined product shipment coordination proposal against a
  production batch. The advisor passes through the caller's own
  claimed weight -- it does NOT invent one, and
  `metaltreatmfg.governor` NEVER trusts it: it independently
...
(def ^:private system-prompt
  (str "あなたは金属処理・表面処理・機械加工工場"
       ...
       "めっき槽・陽極酸化槽・熱処理炉・CNC工作機械等の設備の直接操作(actuate)を絶対に提案してはいけません"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `metaltreatmfg.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :metaltreatmfg-advisor-proposal
   :op         (:op request)
