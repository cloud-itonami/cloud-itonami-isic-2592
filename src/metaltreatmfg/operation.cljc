(ns metaltreatmfg.operation
  "MetalTreatmentOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (MetalTreatmentAdvisor) is sealed into a
  ...
  (:require [langgraph.graph :as g]
            ...
            [metaltreatmfg.advisor :as advisor]
...
(defn build
  "Compiles a MetalTreatmentOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `metaltreatmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; MetalTreatmentAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Metal Treatment, Coating & Machining Plant Operations Governor -- independent censor.
      (g/add-node :govern
