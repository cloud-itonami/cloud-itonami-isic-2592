(ns metaltreatmfg.registry
  "Pure-function domain logic for the metal-treatment/coating/machining
  job-shop plant-operations coordination actor -- equipment/batch
  ...
  pre-existing `kotoba-lang/metaltreatmfg`-style capability library to
  wrap (verified: no such repo exists). The domain logic therefore
  lives here as pure functions, re-verified INDEPENDENTLY by
  `metaltreatmfg.governor` -- the same 'ground truth, not self-report'
  discipline every sibling actor's own registry establishes (e.g.
  `hardwaremfg.registry/shipment-weight-exceeded?` from
  `cloud-itonami-isic-2593`, the closest architectural sibling): never
  ...
  coordinated shipment), not the act of actuating an electroplating
  bath, anodizing tank, heat-treatment furnace or CNC machine, or
  dispatching a real freight carrier (this actor NEVER does either --
  see README `What this actor does NOT do`).

  SCOPE NOTE: ISIC 2592 (this actor) is the metal-treatment/coating/
  machining job shop -- a plant that electroplates, anodizes, powder-
  coats, galvanizes, heat-treats and precision-machines metal parts on
  a fee-or-contract basis. This is distinct from `cloud-itonami-
  isic-2591` (Forging, pressing, stamping and roll-forming of metal --
  the primary heavy metal-forming process), `cloud-itonami-isic-2593`
  (Manufacture of cutlery, hand tools and general hardware -- a
  finished-goods shop), and `cloud-itonami-isic-2599` (Manufacture of
  other fabricated metal products n.e.c.). This actor's own hazard
  profile centers on electroplating-bath chemical-toxicity exposure
  (cyanide, hexavalent-chromium and acid plating/etching baths),
  CNC-machining swarf/cutting-tool laceration hazard, and heat-
  treatment-furnace burn/radiant-heat exposure -- not sheet-metal
  stamping/pressing or forging/roll-forming of bulk metal stock.")

;; ----------------------------- constants -----------------------------

(def valid-product-categories
  "The closed set of product-category values a production-batch (an
  electroplated/anodized/coated/galvanized/heat-treated/machined lot)
  record may declare -- the standard metal-treatment/coating/machining
  job-shop product families this shop produces. Anything else is a
  fabricated/unrecognized product category -- the governor HARD-holds
  rather than let an invented category pass through."
  #{:electroplated-item :anodized-item :powder-coated-item
    :galvanized-item :heat-treated-item :machined-part-item})

(def valid-process-types
  "The closed set of PRIMARY PROCESS types this shop's own production
  line may take -- electroplating, anodizing, powder-coating,
  galvanizing, heat-treatment, or machining. A metal-treatment/
  coating/machining job shop never ships a raw forging or a stamped/
  pressed sheet-metal part (those are a different actor's own
  upstream/sibling scope, not this actor's)."
  #{:electroplating :anodizing :powder-coating
    :galvanizing :heat-treatment :machining})
...
(defn product-category-valid?
  ...
  values (electroplated/anodized/powder-coated/galvanized/heat-
  treated/machined-part family)? nil/blank is treated as invalid (a
...
(defn register-maintenance
  ...
  electroplating-bath/anodizing-tank/heat-treatment-furnace/CNC-
  machine maintenance window against a verified, registered piece of
  equipment. Pure function -- does not actuate the plating/machining
  line or execute any maintenance; it builds the RECORD a shop
  coordinator would keep. `metaltreatmfg.governor` independently
  re-verifies the equipment's own verified/registered ground truth,
  and permanently blocks any attempt to directly actuate the plating/
  machining line (see README `Actuation`), before this is ever
  allowed to commit."
...
(defn register-shipment
  ...
  outbound electroplated/anodized/coated/galvanized/heat-treated/
  machined product shipment against a verified, registered production
  batch. Pure function -- does not dispatch any real freight carrier;
  it builds the RECORD a shop coordinator would keep.
  `metaltreatmfg.governor` independently re-verifies the shipment's
