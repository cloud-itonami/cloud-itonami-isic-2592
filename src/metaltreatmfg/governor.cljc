(ns metaltreatmfg.governor
  "Metal Treatment, Coating & Machining Plant Operations Governor --
  the independent compliance layer that earns the MetalTreatmentAdvisor
  ...
  draft-schedule) the electroplating bath, anodizing tank, heat-
  treatment furnace or CNC machine, whether a shipment proposal's own
  claimed weight would blow through the batch's own logged production
  weight, or when an act stops being a coordination proposal and
  becomes direct plating/machining-line-equipment control, so this
  MUST be a separate system able to *reject* a proposal and fall back
  to HOLD.

  `:itonami.blueprint/governor` is
  `:metal-treatment-coating-machining-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       hallucinated `:plating-bath/
                                       actuate` or `:cnc-machine/run`)
                                       is the 'direct plating/machining-
                                       line-equipment control' scope
                                       violation this actor must NEVER
                                       perform -- HARD, PERMANENT,
                                       unconditional.
    4. Plating/machining-line-
       actuate blocked             -- for `:schedule-maintenance`, does
                                       the proposal's own `:value`
                                       declare `:actuate-plating-
                                       machining-line? true`? Directly
                                       actuating the electroplating
                                       bath, anodizing tank, heat-
                                       treatment furnace or CNC machine
                                       ...
                                       `metaltreatmfg.phase`: this op is
   ...
   11. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [metaltreatmfg.registry :as registry]
...
(def allowed-proposal-effects
  ...
  -- all four are propose-shaped drafts, NEVER a direct plating/
  machining-line-equipment-control effect."
...
(defn- plating-machining-line-control-blocked-violations
  ...
  Anything else (direct electroplating-bath/anodizing-tank/heat-
  treatment-furnace/CNC-machine control, a fabricated actuation
  effect) is this actor's central scope boundary."
  [proposal]
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :plating-machining-line-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") はめっき槽・CNC工作機械等の直接操作に該当する可能性があり、恒久的に禁止")}]))

(defn- plating-machining-line-actuate-blocked-violations
  "HARD, PERMANENT, unconditional: a `:schedule-maintenance` proposal
  whose own `:value` declares `:actuate-plating-machining-line? true`
  is attempting to directly actuate the electroplating bath, anodizing
  tank, heat-treatment furnace or CNC machine -- this actor may only
  ever propose/schedule a DRAFT maintenance window, never actuate the
  plating/machining line directly. No override, ever."
  [{:keys [op]} proposal]
  (when (and (= op :schedule-maintenance)
             (true? (:actuate-plating-machining-line? (:value proposal))))
    [{:rule :plating-machining-line-actuate-blocked
      :detail "めっき槽・陽極酸化槽・熱処理炉・CNC工作機械の直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn check
  "Censors a MetalTreatmentAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (plating-machining-line-control-blocked-violations proposal)
                           (plating-machining-line-actuate-blocked-violations request proposal)
