(ns metaltreatmfg.store
  "SSoT for the metal-treatment/coating/machining job-shop plant-
  ...
  (`cloud-itonami-isic-2593`'s own `hardwaremfg.store`), this build
  ...
  self-contained (no external metal-treatment/coating/machining
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch (an
                             electroplated/anodized/coated/galvanized/
                             heat-treated/machined lot)'s product-
                             category/weight/defect-rate record.
                             ...
                             on file in the shop's production ledger;
                             `:shipped-weight-kg` tracks the batch's
                             own cumulative-shipped ground truth.
    - `equipment`         -- an electroplating-bath/anodizing-tank/
                             heat-treatment-furnace/CNC-machine unit's
                             ...
                             `batches`.
    - `maintenance`       -- a scheduled plating/anodizing/heat-
                             treatment/machining-equipment maintenance-
                             window DRAFT against a piece of equipment
                             (`metaltreatmfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound electroplated/
                             anodized/coated/galvanized/heat-treated/
                             machined product shipment DRAFT
                             (`metaltreatmfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (metaltreatmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (metaltreatmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-category :electroplated-item :process-type :electroplating
                :material "Electroplated Fastener Lot"
                :weight-kg 1200.0 :defect-rate-percent 0.9
                :verified? true :registered? true
                :shipped-weight-kg 200.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-category :machined-part-item :process-type :machining
                :material "CNC-Machined Housing Lot"
                :weight-kg 6000.0 :defect-rate-percent 0.5
                :verified? true :registered? true
                :shipped-weight-kg 5600.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-category :heat-treated-item :process-type :heat-treatment
                :material "Heat-Treated Shaft Lot"
...
(defn- sample-equipment []
  {"plate-001" {:id "plate-001" :kind :electroplating-bath
                :verified? true :registered? true
                :last-maintenance-date "2026-05-01"}
   "cnc-002" {:id "cnc-002" :kind :cnc-machine
              :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `metaltreatmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `metaltreatmfg.registry` and
...
(defn sample-data!
  ...
  coordinated against it); one verified+registered electroplating-bath
  unit (schedulable for maintenance), one UNVERIFIED/unregistered
  CNC-machine unit (blocks any maintenance scheduling against it)
