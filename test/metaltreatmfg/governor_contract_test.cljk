(ns metaltreatmfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control the electroplating bath, anodizing
  tank, heat-treatment furnace or CNC machine directly... does NOT
  authorize or execute maintenance/plating-machining-line operations')
  implemented faithfully. The single invariant under test:

    MetalTreatmentAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Metal Treatment, Coating &
    Machining Plant Operations Governor would reject; `:schedule-
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-category :electroplated-item}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :electroplated-item (:product-category (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-plating-line :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest plating-machining-line-actuate-is-held-and-permanently-blocked
  (testing "a proposal that sets :actuate-plating-machining-line? true -> HOLD, PERMANENT, never reaches request-approval even though the equipment is verified and registered"
    (let [[db actor] (fresh)
      ...
      (is (not= :interrupted (:status res)))
      (is (some #{:plating-machining-line-actuate-blocked} (-> (store/ledger db) last :basis)))
      (is (empty? (store/maintenance-history db))))))
  ...
  (testing "scheduling the SAME maintenance record twice -> HOLD on the second attempt"
    (let [[db actor] (fresh)
          _ (exec-op actor "t9a" {:op :schedule-maintenance :effect :propose :subject "mnt-1"
                                  :value {:equipment-id "plate-001" :maintenance-type :bath-chemistry-refresh
                                          :scheduled-date "2026-08-01" :actuate-plating-machining-line? false}} coordinator)
          _ (approve! actor "t9a")
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "plate-001" :severity :moderate
                                            :description "electroplating bath ventilation hood underperforming"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "plate-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
