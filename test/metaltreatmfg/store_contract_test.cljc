(ns metaltreatmfg.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `metaltreatmfg.store` ns docstring for why a second (Datomic-backed)
...
(deftest sample-data-read-basics
  (let [s (seeded)]
    ...
    (is (true? (:verified? (store/equipment-unit s "plate-001"))))
    (is (true? (:registered? (store/equipment-unit s "plate-001"))))
    (is (false? (:verified? (store/equipment-unit s "cnc-002"))))
    (is (false? (:registered? (store/equipment-unit s "cnc-002"))))
    (is (= ["cnc-002" "plate-001"] (mapv :id (store/all-equipment s))))
    (is (= [] (store/ledger s)))
...
(deftest fresh-store-has-no-batches-or-equipment
  (let [s (store/mem-store)]
    ...
    (is (nil? (store/equipment-unit s "plate-001")))))

(deftest batch-upsert-merges-preserving-untouched-fields
  (let [s (seeded)]
    (store/commit-record! s {:effect :batch/upsert :path ["batch-001"]
                             :value {:product-category :machined-part-item}})
    (is (= :machined-part-item (:product-category (store/batch s "batch-001"))))
...
(deftest maintenance-schedule-commits-and-advances-sequence
  (testing "commit-record! (like every sibling actor's own MemStore) returns the store `s`, not the domain result -- inspect the store directly, matching the discipline the actor's own :commit node relies on"
    (let [s (seeded)]
      ...
      (is (= "plate-001" (:equipment-id (store/maintenance s "mnt-1"))))
...
(deftest safety-concern-flag-appends
  (let [s (seeded)]
    ...
    (store/commit-record! s {:effect :safety-concern/flag :path ["concern-2"]
                             :value {:equipment-id "cnc-002" :severity :high}})
