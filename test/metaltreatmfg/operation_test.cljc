(ns metaltreatmfg.operation-test
  "Smoke tests for the compiled MetalTreatmentOperationActor graph
  itself (build + one happy path per op). The governor's full rule
  contract (HARD holds, escalation, phase gating) is exercised in
  `metaltreatmfg.governor-contract-test`; the Store contract in
  `metaltreatmfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "MetalTreatmentOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "plate-001" :severity :moderate :description "cyanide bath ventilation hood underperforming"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
